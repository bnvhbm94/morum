...
export async function openHarness(config){
 ...
 assert.equal((await admin.query('select migration_tag from knowledge.schema_info where singleton')).rows[0].migration_tag,'stage05-deferred-trigger-security');}catch(error){await Promise.allSettled(clients.map(c=>c.end()));throw error;}
