...
if(!config){test('Stage03 PostgreSQL integration NOT RUN',{skip:'No acknowledged disposable local PostgreSQL; no substitute database'},()=>{});}
 ...
 test('Stage03 real database functional acceptance',async t=>{
  ...
  try{
   const c=await pool.connect();try{await assertTestDatabase(c,config);const r=(await c.query("select migration_tag,contract_version from knowledge.schema_info where singleton")).rows[0];assert.equal(r.migration_tag,'stage05-deferred-trigger-security');assert.equal(r.contract_version,'2.1.0');}finally{c.release();}
