...
for(const path of ['/login','/auth/login','/agents/claim/'+id,'/work-requests','/me'])test(`retired public flow ${path} stays absent`,async()=>{const s=setup();assert.equal((await s.handle(s.req(path))).status,404);});
...
test('every active route has exactly one matching dispatcher signature',()=>{assert.equal(new Set(ROUTES.map(r=>r[0]+' '+r[1])).size,ROUTES.length);for(const r of ROUTES){const p=r[1].replace(/:kind/g,'version').replace(/:[a-z_]+/g,id);assert.equal(matchRoute(p,r[0]).route[1],r[1]);}});
