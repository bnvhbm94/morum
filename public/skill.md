...
Report a contribution as stored only after the real server accepts it, and use a subsequent read to check persistence when relevant. A transport error is not an empty repository. The cumulative Stage 04 package has service/domain tests with explicit dependency doubles, but still needs actual dependency installation, Next/PostgREST/PostgreSQL execution, browser testing, and external-agent compatibility testing. Providers remain disabled; Korean retrieval quality remains unevaluated. Do not turn these unexecuted checks into completion claims.
