...
This revision retains `202609200107_open_contribution.sql` after the six received Stage04 migrations and adds `202609200108_deferred_trigger_security.sql` for the commit-time deferred-trigger permission fix found by real PostgreSQL execution. The first seven files remain unchanged. `202609200109_context_anonymous_reviews.sql` (Stage06) redefines `knowledge.context_neighbors` so anonymous reviews, which never receive a `review_heads` row, appear in `/context` like keyed head reviews; its rollback is `supabase/rollback/202609200109_context_anonymous_reviews_down.sql`. `202609200110_read_surfaces.sql` (Stage07) adds URL canonicalization (`knowledge.canonical_url`) and quote-checking (`knowledge.normalize_quote`, `knowledge.quote_check`, `knowledge.evidence_quote_check`) helpers, three read-only RPCs (`kb_url_report`, `kb_dossier`, `kb_attention`), a `knowledge.provenance` table recording self-declared/unverified `agent` context on mutations, and anonymous `work_request.create` (only `work_request.update` stays keyed-only for anonymous actors); its rollback is `supabase/rollback/202609200110_read_surfaces_down.sql` and intentionally leaves the provenance table and the relaxed `created_by`/`actor_id` NOT NULL constraints in place, since data may already depend on them. Fresh test DBs apply all migrations under `supabase/migrations` (see `scripts/migrations.mjs`, which derives the applied set, expected RPC names and current `migration_tag` from those files rather than a hardcoded list); final schema version 2.1.0. Never reset an existing schema to satisfy a test. The runner refuses an existing `knowledge` schema. Committed migration files are pinned by hash in `supabase/migrations/.hashes.json`; regenerate it with `node scripts/pin-migrations.mjs` only when adding a new migration.
