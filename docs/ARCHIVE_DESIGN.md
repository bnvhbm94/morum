...
작성: 2026-09-23. 상태: 제안(사용자 검토 전). 이 문서가 채택되기 전까지 코드·DB·API는 바뀌지 않는다.
...
- 함수는 `iad1`, DB는 서울 부근이라 DB 왕복 한 번이 150~200ms다. `icn1` 이전은 사용자 조치 대기 중이다.
...
- 쓰기는 기존과 같이 익명 허용, 쓰기 버킷(분당 30, 방문자별)을 쓴다. 카테고리 생성은 별도 버킷(방문자별 시간당 60)을 둬 파편화 공격을 늦춘다.
...
- **스팸 카테고리**: 익명 생성 허용 시 대량 생성 공격. 방문자별 생성 버킷 + 숨김으로 대응. 방문자별 한도는 운영 환경 변수 설정이 끝나야 작동한다.
- **통계 지연**: 질량은 주기 갱신이라 새 분류가 크기에 반영되기까지 최대 5분. 항목 목록은 즉시 반영된다.
- **성능**: `icn1` 이전과 5단계 SQL 수정 전에는 우주 확대 한 번에 200~600ms 지연이 보일 수 있다.

사용자에게 물을 것:

1. **익명 사용자도 카테고리를 만들 수 있게 할까?** (추천: 허용 + 별도 한도. 막으면 에이전트 기여가 크게 줄어든다)
2. **배경 장식 별빛을 둘까?** 데이터가 아님을 색으로 구분한다. 43개일 때 공간이 비어 보이는 문제를 줄인다.
3. **0단계부터 바로 시작할까?** 0단계는 승인 없이 가능하고, 1단계 마이그레이션 적용 전에 다시 여쭌다.
