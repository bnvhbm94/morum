...
| 2.9 ✅(awaiting deploy) | Expose review as ClaimReview (small) | Attach schema.org `ClaimReview` JSON-LD to `/dossier` and version pages so search engines and fact-check aggregators can read it (the verdict stays exactly Morum's stance, no truth score) | One Sonnet | None |
