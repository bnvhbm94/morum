-- Stage12 / excerpt length cap. Additive only: no table/column drop, no
-- truncate, no extension install, no role alteration. Contract stays 2.1.0.
-- knowledge.sources.submitted_text allowed 100,000 chars while
-- LICENSE-DATA.md describes third-party text as "short quotations".
-- Excerpts are the quoted passage plus surrounding context, not whole
-- articles: cap new rows at 8,000 code points.
--  1. A CHECK constraint on knowledge.sources enforces the cap for all
--     future writes. It is added NOT VALID so existing rows (append-only
--     ledger) are never touched or re-scanned.
--  2. knowledge.validate_command (defined 202609200102, re-created
--     202609200112 for quote selectors; this is its latest definition) is
--     re-created verbatim except the source.create submitted_text limit,
--     so an over-length excerpt fails during RPC validation with the usual
--     VALIDATION_FAILED error instead of surfacing a raw constraint
--     violation.
-- Rollback: supabase/rollback/202609200116_excerpt_length_cap_down.sql
BEGIN;

-- NOT VALID: enforces the cap on new/updated rows only; existing rows in
-- this append-only ledger are left unvalidated and untouched.
ALTER TABLE knowledge.sources ADD CONSTRAINT sources_submitted_text_excerpt_cap CHECK (submitted_text IS NULL OR pg_catalog.length(submitted_text) <= 8000) NOT VALID;

-- === knowledge.validate_command (copy of 202609200112's body; only
-- source.create's submitted_text limit changes, 100000 -> 8000) =========
CREATE OR REPLACE FUNCTION knowledge.validate_command(op text,j jsonb) RETURNS void LANGUAGE plpgsql STABLE SET search_path='' AS $$ DECLARE fields text[]; x jsonb; t text; BEGIN
 ...
 WHEN 'source.create' THEN
  ...
  PERFORM knowledge.jtext(j->'title',240,1,true);PERFORM knowledge.jtext(j->'submitted_text',8000,0,true);
