-- Stage10 / dossier anchor evidence fix. Additive only: no table/column
-- drop, no truncate, no extension install, no role alteration. Contract
-- stays 2.1.0.
-- Fixes a confirmed production defect in knowledge.kb_dossier (latest body
-- from 202609200113): evidence was matched against ev.target_version_id=vid
-- only, in four places (evidence_total, the evidence list, and the two
-- internal-evidence subqueries feeding premises_total/premises). Evidence
-- attached to an anchor of the version (ev.target_anchor_id in the
-- version's already-computed anchor_ids) was silently dropped. Example:
-- caffeine v2 9a66a918-9a73-44dd-a4d8-32ecd9987c64 has 3 evidence rows via
-- url-report but kb_dossier returned 1 with omitted.evidence=0.
-- Fix: re-create kb_dossier (copy of 202609200113's body verbatim) with
-- each of those four predicates changed to
-- (ev.target_version_id=vid OR ev.target_anchor_id=ANY(anchor_ids)).
-- Nothing else changes.
-- Rollback: supabase/rollback/202609200114_dossier_anchor_evidence_down.sql
BEGIN;
...
DECLARE target jsonb;vid uuid;v knowledge.versions%ROWTYPE;blind boolean;anchor_ids uuid[];
 ...
 SELECT count(*) INTO premises_total FROM (
  SELECT DISTINCT ON (pv) pv FROM (
   ...
   FROM knowledge.evidence ev WHERE ev.kind='internal' AND (ev.target_version_id=vid OR ev.target_anchor_id=ANY(anchor_ids)) AND knowledge.is_public('evidence',ev.id)
