-- Rollback for 202609200113_dossier_agreements.sql.
-- Restores the 202609200110 body of public.kb_dossier verbatim: agreements
-- reverts to {agree_keyed, agree_anonymous} only (no reviews/truncated), and
-- every reviews listing's 'on' field reverts to the bare {kind,id} shape
-- from knowledge.row_ref (no anchor exact/start/end). Drops the
-- knowledge.review_on_ref helper 202609200113 added.
BEGIN;
...
DROP FUNCTION IF EXISTS knowledge.review_on_ref(knowledge.reviews);
