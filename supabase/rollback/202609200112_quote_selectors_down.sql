-- Rollback for 202609200112_quote_selectors.sql.
-- Restores stage07 behavior: drops public.kb_locate, knowledge.resolve_selector
-- and knowledge.locate_quote, and restores the 202609200110 body of
-- knowledge.mutate verbatim (anchor.create/version.create go back to strict
-- start/end-only selectors; a quote-only selector or edit fails at the
-- missing start/end instead of being resolved).
-- knowledge.validate_command is intentionally LEFT as redefined by
-- 202609200112: it still accepts the optional start/end/prefix/suffix shape.
-- Narrowing an already-loosened structural check back down is not itself a
-- safe additive-only rollback action (same reasoning as 202609200110's own
-- rollback leaving the provenance table and relaxed NOT NULL constraints in
-- place); the effect is limited to a validation-time acceptance of a shape
-- that knowledge.mutate, once restored below, no longer resolves.
BEGIN;

DROP FUNCTION IF EXISTS public.kb_locate(jsonb);
DROP FUNCTION IF EXISTS knowledge.resolve_selector(text,jsonb);
DROP FUNCTION IF EXISTS knowledge.locate_quote(text,text,text,text,integer);
